...
pub fn handle_settings(
    ...
    match setting {
        ...
        Some(super::SettingsCmd::Ip { value }) => {
            ...
            save_and_log(config, config_path, &format!("IP прослушивания: {}", config.listen_ip))?;
        }
        ...
        Some(super::SettingsCmd::Method { value }) => {
            if value != "random" && value != "fastest" { bail!("Метод должен быть random или fastest"); }
            config.select_mode = value;
            save_and_log(config, config_path, &format!("Метод выбора: {}", config.select_mode))?;
        }
        ...
        Some(super::SettingsCmd::Info { on }) => {
            ...
            save_and_log(config, config_path, &format!("Показывать инфо при обновлении: {}", if on { "вкл" } else { "выкл" }))?;
        }
        Some(super::SettingsCmd::Parallel { value }) => {
            config.parallel_tests = value;
            save_and_log(config, config_path, &format!("Параллельных тестов: {}", value))?;
        }
        ...
        Some(super::SettingsCmd::BlacklistStrikes { strikes }) => {
            ...
            save_and_log(config, config_path, &format!("Ошибок до блэклиста: {}", strikes))?;
        }
        Some(super::SettingsCmd::AutoUpdate { interval_min, ids }) => {
            ...
            save_and_log(config, config_path, &format!("Автообновление: {} мин для {}", config.auto_update_interval, config.auto_update_ids))?;
        }
        Some(super::SettingsCmd::AutoMenuUpdate { enable, interval_sec }) => {
            config.auto_menu_update_enabled = enable;
            config.auto_menu_update_interval = interval_sec;
            let status = if enable { format!("включено, интервал {} с", interval_sec) } else { "выключено".into() };
            save_and_log(config, config_path, &format!("Автообновление меню: {}", status))?;
        }
...
fn show_current_settings(config: &AppConfig) {
    ...
    println!("  Insecure:        {}", if config.insecure { "вкл" } else { "выкл" });
    println!("  Speedtest:       {}", if config.speedtest { "вкл" } else { "выкл" });
    println!("  HTTP verbose:    {}", if config.http_verbose { "вкл" } else { "выкл" });
    println!("  Показ инфо при обн.: {}", if config.show_update_info { "вкл" } else { "выкл" });
    println!("  Параллельные тесты: {}", config.parallel_tests);
    println!("  Блэклист:        ошибок {}, длит. {} с", config.blacklist_strikes, config.blacklist_duration);
    println!("  Автообновление:  {} мин, IDs: {}", config.auto_update_interval, config.auto_update_ids);
    println!("  Автообновление меню: {}", if config.auto_menu_update_enabled { format!("{} с", config.auto_menu_update_interval) } else { "выкл".into() });
